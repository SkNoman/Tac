...
class GeneralRepository @Inject constructor(private val api: ApiInterface) {
    ...
    fun submitPaymentInfo(url:String, jsonObject: JsonObject, onCallback: SubmitPaymentInfoCallback){
        api.submitPaymentInfo(url,jsonObject).enqueue(object: retrofit2.Callback<SubmitPaymentInfoResponse>{
            override fun onResponse(
                call: Call<SubmitPaymentInfoResponse>,
                response: Response<SubmitPaymentInfoResponse>
            ) {
                if (response.code() == 200){
                    onCallback.onPaymentInfoResponse(response.body()!!)
                }else if (response.code() == 400){
            ...
            override fun onFailure(call: Call<SubmitPaymentInfoResponse>, t: Throwable) {
                onCallback.onErrorRes(ErrorResponse(-1, Constant.ERROR_MESSAGE))
            }
        })
    }
